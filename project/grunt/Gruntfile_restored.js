...
module.exports = function(grunt){
	...
	grunt.initConfig({    
		concat: {
			options: {
				separator: '\n',
				sourceMap: true,
				banner: "/*Processed by SNA "+datetime+"*/\n"
			},      
			css: {
				src: ['../css/**/*.css'],
				dest: 'dist/style.css',
		...
		obfuscator: {
			options: {
				banner: "// obfuscated with grunt-contrib-obfuscator. "+datetime+"\n",
				debugProtection: true,
				debugProtectionInterval: true,
				// domainLock: ['nrl.selfmade.one']
			},
			task1: {
				options: {
	...
	// grunt.registerTask('build',['concat','cssmin','uglify','watch']);    
};
